    ...
    for n in range(n_train):
        ...
        while(ti == tj):
            ...
            if ti < tj: #pick tj snippet to be later than ti
                ti_start = np.random.randint(len(demonstrations[ti])-2*snippet_length)
                tj_start = np.random.randint(ti_start, len(demonstrations[tj])-snippet_length)
            else: #ti is better so pick later snippet in ti
                tj_start = np.random.randint(len(demonstrations[ti])-2*snippet_length)
                ti_start = np.random.randint(tj_start, len(demonstrations[tj])-snippet_length)
