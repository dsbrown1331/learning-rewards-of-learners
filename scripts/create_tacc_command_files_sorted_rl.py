def create_command_file(env_name):
    ...
    command_str += "SINGULARITYENV_OPENAI_LOG_FORMAT='stdout,log,csv,tensorboard' SINGULARITYENV_OPENAI_LOGDIR=$WORK/tflogs/"+ env_name + "_sorted_15_25-ppo-2 singularity exec --nv ${SINGULARITY_CACHEDIR}/tacc-maverick-ml-latest.simg python -m baselines.run --alg=ppo2 --env=" + env_id + " --custom_reward pytorch --custom_reward_path $WORK/output/learned_rewards/" + env_name + "_15_sorted_pref.params --num_timesteps=5e7 --save_interval=200\n"
    print(command_str)
